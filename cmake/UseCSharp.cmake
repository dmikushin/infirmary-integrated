#
# Adopted from https://github.com/vgvassilev/solid
#
    ...
    elseif (should_not_skip)
      ...
      foreach(s ${sources})
        if ("${s}" MATCHES "\\.(stetic)$")
          ...
          endforeach()
        elseif ("${s}" MATCHES "\\.resx$")
          # Expand wildcards
          FILE( GLOB s_glob ${s} )
          foreach (res ${s_glob})
            unset(resgen_status CACHE)
            execute_process(COMMAND resgen /useSourcePath "${res}" RESULT_VARIABLE resgen_status)
            if (NOT ${resgen_status} EQUAL 0)
              message(FATAL_ERROR "resgen returned ${resgen_status}")
            endif()
            STRING(REGEX REPLACE "\.resx$" ".resources" res ${res})
            set(parent_cmake "")
            unset(parent_cmake_folder CACHE)
            get_filename_component(parent_cmake_folder "${res}" DIRECTORY)
            while("${parent_cmake}" STREQUAL "")
              unset(parent_cmake_candidate CACHE)
              find_path(parent_cmake_candidate CMakeLists.txt "${parent_cmake_folder}")
              if (NOT ${parent_cmake_candidate} STREQUAL "parent_cmake_candidate-NOTFOUND")
                set(parent_cmake "${parent_cmake_candidate}")
              endif()
              get_filename_component(parent_cmake_folder "${parent_cmake_folder}" DIRECTORY)
            endwhile("${parent_cmake}" STREQUAL "")
            get_filename_component(parent_cmake_folder "${parent_cmake}" DIRECTORY)
            STRING(REGEX REPLACE "${parent_cmake_folder}/" "" res_name ${res})
            STRING(REGEX REPLACE "/" "." res_name ${res_name})
            list(APPEND separated_embd_resources "/resource:${res},${res_name}")
          endforeach()
      ...
      string(SUBSTRING "${GEPLRE_LastDate}" 0 4 GEPLRE_LastYear)
      # Project meta data
      set(meta_idx 1)
      ...
      MESSAGE( STATUS "Adding C# ${type} ${name}: '${CSHARP_COMPILER} /t:${output_type} /out:${out} /doc:${out}.xml ${platform_param} ${CSBUILDFLAGS} ${CSHARP_SDK_COMPILER} ${separated_embd_resources} ${separated_sources} ${processed_refs}'" )
      # Transform the ;-separated lists into ' '-separated. This helps copy paste of the command on the terminal
      set(ESCAPED_COMMENT "Compiling C# ${type} ${name}: '${CSHARP_COMPILER} /t:${output_type} /out:${out} /doc:${out}.xml ${platform_param} ${CSBUILDFLAGS} ${CSHARP_SDK_COMPILER} ${separated_embd_resources} ${separated_sources} ${processed_refs}'")
